...
const ZakatCalculatorPage: React.FC = () => {
        ...
        <div>
            ...
            <div className="relative">
                 <input type="number" id={name} name={name} value={assets[name]} onChange={handleAssetChange} placeholder={placeholder}
                    className="w-full p-3 bg-gray-100 dark:bg-gray-700 rounded-lg pr-12 focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {/* Calculator Inputs */}
            <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg space-y-6">
                <h2 className="text-3xl font-bold text-teal-800 dark:text-teal-300 border-b pb-4 mb-6">حاسبة الزكاة</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t mt-6">
                    <div>
                        ...
                        <input type="number" id="goldPrice" value={goldPrice} onChange={e => setGoldPrice(Number(e.target.value))} className="w-full p-3 bg-gray-100 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" />
                    </div>
                    <div>
                        <label htmlFor="silverPrice" className="block mb-2 font-medium text-gray-700 dark:text-gray-300">سعر جرام الفضة ($)</label>
                        <input type="number" id="silverPrice" value={silverPrice} onChange={e => setSilverPrice(Number(e.target.value))} className="w-full p-3 bg-gray-100 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" />
                    </div>
                 </div>
            ...
            {/* Results */}
            <div className="bg-teal-600 text-white p-8 rounded-xl shadow-lg flex flex-col justify-center space-y-6">
                <h2 className="text-3xl font-bold border-b border-teal-400 pb-4">النتيجة</h2>
                ...
                <div className="mt-6 pt-6 border-t border-teal-400 text-center">
                    {isLiable ? (
                        <>
                            <p className="text-lg text-teal-100">لقد بلغت النصاب، تجب عليك الزكاة.</p>
                            <p className="text-5xl font-extrabold text-white mt-2">${zakatAmount.toFixed(2)}</p>
                            <p className="text-sm text-teal-200 mt-1">(2.5% من إجمالي الأصول)</p>
                        </>
                    ) : (
                        <p className="text-xl font-bold text-teal-100">لم تبلغ النصاب، لا تجب عليك الزكاة حاليًا.</p>
                    )}
                </div>
            </div>
        </div>
