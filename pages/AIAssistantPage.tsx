...
const AIAssistantPage: React.FC = () => {
            ...
            <div>
                {lines.map((line, lineIndex) => {
                    ...
                    if (line.trim().startsWith('* ')) {
                         return <div key={lineIndex} className="flex items-start"><span className="mr-2 mt-1">•</span><span>{line.substring(2)}</span></div>;
                    }

                    return (
                        <p key={lineIndex} className={lines.length > 1 ? 'mb-2' : ''}>
    ...
    return (
        <div className="flex flex-col min-h-[75vh] bg-white dark:bg-gray-800 rounded-xl shadow-lg" dir="rtl">
            <div ref={chatContainerRef} className="flex-1 p-6 overflow-y-auto space-y-6">
                {messages.length === 0 && (
                    <div className="text-center animate-fade-in">
                        <i className="ph-fill ph-brain text-6xl text-teal-500 mb-4"></i>
                        ...
                        <div className="mt-6 flex flex-wrap justify-center gap-2">
                            {suggestionPrompts.map(prompt => (
                                <button key={prompt} onClick={() => handleSend(prompt)} className="bg-teal-50 dark:bg-gray-700 text-teal-700 dark:text-teal-300 px-4 py-2 rounded-full text-sm hover:bg-teal-100 dark:hover:bg-gray-600 transition-colors">
                                    {prompt}
                                </button>
                            ))}
                        </div>
                ...
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-4 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                         {msg.role === 'model' && <i className="ph-fill ph-brain text-2xl text-teal-500 mt-2"></i>}
                        <div className={`max-w-xl p-3 sm:p-4 rounded-2xl ${msg.role === 'user' ? 'bg-teal-600 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none'}`}>
            ...
            <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                <form onSubmit={handleSubmit} className="flex items-center gap-4">
                    <input
                        ...
                        disabled={isLoading}
                        className="flex-1 w-full p-3 bg-gray-100 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:opacity-50"
                    />
                    <button type="submit" disabled={isLoading || !input.trim()} className="p-3 bg-teal-600 text-white rounded-lg hover:bg-teal-700 disabled:bg-teal-400 disabled:cursor-not-allowed transition-colors">
