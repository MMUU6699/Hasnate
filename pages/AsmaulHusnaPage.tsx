...
const NameCard: React.FC<{ name: NameOfAllah }> = ({ name }) => (
  <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-lg flex flex-col items-center justify-center text-center transform hover:-translate-y-1 transition-transform duration-300 border-t-4 border-teal-400 dark:border-teal-600">
    <span className="text-gray-500 dark:text-gray-400 text-sm mb-2">#{name.number}</span>
    <h3 className="text-3xl font-arabic font-bold text-teal-700 dark:text-teal-400 mb-1">{name.name}</h3>
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <h2 className="text-4xl font-bold text-teal-800 dark:text-teal-300">أسماء الله الحسنى</h2>
