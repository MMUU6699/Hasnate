    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-4 sm:p-6 lg:p-8 animate-fade-in">
            <div className="text-center border-b border-gray-200 dark:border-gray-700 pb-6 mb-6">
                <h2 className="text-4xl font-bold text-teal-800 dark:text-teal-300">مواقيت الصلاة</h2>
            ...
            {prayerTimes && (
                <div className="space-y-4">
                    {Object.keys(prayerNames).map((key) => (
                        <div key={key} className={`flex justify-between items-center p-4 rounded-lg transition-all duration-300 ${nextPrayer === key ? 'bg-teal-600 text-white shadow-lg scale-105' : 'bg-gray-100 dark:bg-gray-700'}`}>
                            <div className="flex items-center">
                                <i className={`ph-fill ph-${key === 'sunrise' ? 'sun' : 'moon'} text-2xl mr-4 ${nextPrayer === key ? 'text-white' : 'text-teal-500'}`}></i>
                                <span className={`font-bold text-lg ${nextPrayer === key ? 'text-white' : 'text-gray-800 dark:text-gray-200'}`}>{prayerNames[key]}</span>
                            </div>
                            <span className={`font-mono text-2xl font-semibold ${nextPrayer === key ? 'text-teal-100' : 'text-gray-700 dark:text-gray-300'}`}>
                                {prayerTimes[key]}
                            </span>
                        </div>
                    ))}
