...
const HadithPage: React.FC = () => {
  ...
  if (selectedChapter) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg animate-fade-in">
        <div className="p-4 border-b dark:border-gray-700 flex items-center sticky top-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm z-10">
          ...
          <div>
            <h2 className="text-lg font-bold text-teal-800 dark:text-teal-400">{selectedChapter.name}</h2>
            <p className="text-sm text-gray-600 dark:text-gray-300">{selectedBook.name}</p>
          </div>
        </div>
        <div className="p-6 space-y-6">
          {selectedChapter.hadiths.map(hadith => (
            <div key={hadith.id} className="p-4 border-r-4 border-teal-500 bg-gray-50 dark:bg-gray-900/40 rounded-lg">
              <p className="text-lg leading-relaxed text-gray-800 dark:text-gray-200">{hadith.text}</p>
              {hadith.narrator && <p className="text-sm text-teal-600 dark:text-teal-400 mt-3 font-medium">الراوي: {hadith.narrator}</p>}
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg animate-fade-in">
      <div className="text-center p-8 border-b border-gray-200 dark:border-gray-700">
        <h2 className="text-4xl font-bold text-teal-800 dark:text-teal-300">مكتبة الحديث</h2>
        <p className="text-lg text-gray-600 dark:text-gray-400 mt-2">{selectedBook.name}</p>
      </div>
      <ul className="divide-y divide-gray-200 dark:divide-gray-700">
        {selectedBook.chapters.map(chapter => (
          <li key={chapter.id}>
            <button onClick={() => handleSelectChapter(chapter)} className="w-full text-right p-5 hover:bg-teal-50 dark:hover:bg-gray-700/50 transition-colors flex justify-between items-center">
              <div>
