...
const FeatureCard: React.FC<FeatureCardProps> = ({ to, icon, title, description }) => (
  <Link to={to} className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-lg hover:shadow-2xl dark:hover:shadow-teal-900/50 hover:-translate-y-2 transition-all duration-300 ease-in-out flex flex-col items-center text-center group">
      <div className="bg-teal-100 dark:bg-teal-900/50 text-teal-600 dark:text-teal-400 p-3 sm:p-4 rounded-full mb-4 transition-colors duration-300 group-hover:bg-teal-600 dark:group-hover:bg-teal-500 group-hover:text-white dark:group-hover:text-white">
        <i className={`ph-fill ph-${icon} text-3xl sm:text-4xl`}></i>
      </div>
      <h3 className="text-xl font-bold text-teal-800 dark:text-teal-300 mb-2">{title}</h3>
...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      <div className="p-8 bg-gradient-to-br from-teal-600 to-cyan-600 rounded-2xl text-white shadow-xl dark:shadow-2xl dark:shadow-teal-900/50">
        <h1 className="text-4xl font-bold">أهلاً بك في حسناتي</h1>
        <p className="text-lg mt-2 text-teal-100">"ألا بذكر الله تطمئن القلوب"</p>
        <p className="text-lg mt-4 font-semibold">{islamicDate}</p>
      </div>
