...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
      ...
      <NavLink
        ...
        className={`flex items-center justify-end p-3 my-1 rounded-lg transition-colors duration-200 ${
          isActive
            ...
            : 'text-gray-600 dark:text-gray-300 hover:bg-teal-50 dark:hover:bg-gray-700 hover:text-teal-700 dark:hover:text-white'
        }`}
      >
        <div className="flex items-center">
            {item.online && <span className="text-xs bg-sky-500 text-white rounded-full px-2 py-0.5 ml-2 font-mono">Online</span>}
  ...
  return (
    <aside 
      className={`fixed inset-y-0 right-0 z-50 w-72 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-l border-gray-200 dark:border-gray-700 p-4 flex flex-col h-screen transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      dir="rtl"
    >
      <div className="flex items-center justify-between text-center py-6">
        <div className="flex-grow">
          <h1 className="text-3xl font-bold text-teal-600 dark:text-teal-400">حسناتي</h1>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <button
          onClick={logout}
          className="w-full flex items-center justify-end p-3 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-red-100 dark:hover:bg-red-900/50 hover:text-red-700 dark:hover:text-red-400 transition-colors duration-200"
        >
